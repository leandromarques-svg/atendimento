...
export const parseDurationToSeconds = (durationStr: string): number => {
  ...
  // Detect BLIP format: 0d 00:01:08
  const blipRegex = /^(\d+)d\s+(\d{2}):(\d{2}):(\d{2})$/;
  const blipShortRegex = /^(\d+)d\s+(\d{2}):(\d{2})$/;
  if (blipRegex.test(durationStr)) {
    const [, days, hours, minutes, seconds] = durationStr.match(blipRegex)!;
    return (
      parseInt(days) * 86400 +
      parseInt(hours) * 3600 +
      parseInt(minutes) * 60 +
  ...
  } else if (blipShortRegex.test(durationStr)) {
    // Sometimes BLIP may omit seconds (e.g., 0d 00:01)
    const [, days, hours, minutes] = durationStr.match(blipShortRegex)!;
    return (
      parseInt(days) * 86400 +
      parseInt(hours) * 3600 +
      parseInt(minutes) * 60
  ...
  // FRESH format: 3h 9m 41s 500ms (and similar)
