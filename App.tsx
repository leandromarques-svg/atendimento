...
const App: React.FC = () => {
  ...
  // Registros inconsistentes: tempos zerados ou resolvidos em segundos (<= 1s)
  const inconsistentRecords = useMemo(() => {
    return filteredData.filter(d => d.ahtSeconds <= 1 || d.frtSeconds <= 1);
  }, [filteredData]);

  const metrics = useMemo(() => {
    if (filteredData.length === 0) return null;

    // Filtrar apenas registros com tempos válidos (maiores que 1 segundo)
    const validAHT = filteredData.filter(d => d.ahtSeconds > 1);
    const validFRT = filteredData.filter(d => d.frtSeconds > 1);
  ...
  const generateAIInsights = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Erro desconhecido ao chamar backend Gemini');
      }
      const data = await response.json();
      const text = data.text;
      console.log('[IA] Texto recebido do backend:', text);
      ...
      alert(`Erro ao conectar com a IA: ${errorMsg}\n\nVerifique o backend e veja detalhes no console do navegador para diagnóstico.`);
    } finally {
                ...
                <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                  ...
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col relative overflow-hidden group hover:shadow-md transition-all">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-red-400 text-[10px] font-black uppercase tracking-widest">Inconsistentes</p>
                        <h4 className="text-3xl font-black text-red-600 mt-1">{inconsistentRecords.length}</h4>
                      </div>
                      <div className="p-3 bg-red-50 rounded-xl">
                        <Activity className="text-red-500" size={18} />
                      </div>
                    </div>
                    <div className="text-[10px] text-red-500 mt-auto">Tempos zerados ou resolvidos em segundos (&le; 1s)</div>
                  </div>
                  <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col relative overflow-hidden group hover:shadow-md transition-all">
                    <div className="flex justify-between items-start mb-4">
                      <div>
